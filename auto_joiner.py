...
def check_teacher_left_meeting(list1, list2):
    result = False
    for x in list1:
        for y in list2:
            if x == y:
                print(f'{x} lecturing in meeting...')
                result = True
                return result
    # print(list1)
    return result
...
class Team:
    ...
    def update_meetings(self):        
        try:
            ...
            print('Cannot refresh page')
        global users_in_call
        active_callers_name_list = []
        hidden_active_callers_name_list = []
                    ...
                    for call_elem in all_call_elems:
                        try:
                            ...
                            if header_id is not None:
                                ...
                                active_callers_name = call_elem.find_elements_by_css_selector("div > calling-live-roster > .ts-calling-live-roster > div[role='listitem']")
                                hidden_active_callers_name = call_elem.find_elements_by_css_selector("div > calling-live-roster > .ts-calling-live-roster > div[class='item hidden-participants']")
                                hidden = [hidden.get_attribute("aria-label") for hidden in hidden_active_callers_name]
                                if len(hidden) != 0:
                                    hidden_active_callers_name_list = hidden.split(', ')
                                else:
                                    hidden_active_callers_name_list = []
                                hidden_active_callers_name_list = []
                                print("Only 4 people in meeting")
                                
                                for callers in active_callers_name:
                                    active_callers_name_list.append(callers.get_attribute("aria-label"))
                                    print(callers)
                                users_in_call = active_callers_name_list + hidden_active_callers_name_list
                                # print(users_in_call)
                                break

                    if meeting_id == active_meeting.meeting_id:
                        if check_teacher_left_meeting(users_in_call, config['teachers_name']):
...
def main():
    ...
    if window_size['width'] < 1920:
        ...
        browser.set_window_size(config['window_width'], window_size['width'])
